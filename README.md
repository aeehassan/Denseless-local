...
**4. Run the AI**
There are two ways to run and interact with Denseless, depending on your goal:

### Option A: The Streamlit Interface (Recommended for Testers)

This is the easiest and most interactive way to experience the system. It was designed specifically for seamless UI interaction and testing.

* **Run command:** Execute `streamlit run frontend.py` in your terminal.
* **Test Profile:** This environment uses student profile **1001**. You are free to create a new profile. The system will do the needful.
* **Note on JSON Schema:** The schema for the Streamlit environment differs slightly from the notebook environment. It includes explicit file paths (e.g., `quiz_path`) to track exactly where generated assets are stored locally to handle UI state management.

### Option B: The Interactive Notebook (Research & Evaluation)

All foundational research, system evaluation, and algorithm testing were conducted using scripts and this notebook interface.

* **Launch:** Open `DEMO.ipynb` in your preferred notebook environment.
* **Test Profile:** This environment uses student profile **1019**. You are free to create a new profile. The system will do the needful.
* **Instructions:** Execute the notebook cells sequentially, one by one. Make sure to modify the parameter values where explicitly instructed by the inline comments in a notebook cell. Following these instructions will ensure the system works exactly as intended.
